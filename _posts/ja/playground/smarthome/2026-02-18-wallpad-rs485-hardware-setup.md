...
4. 左側メニューからWiFi設定に移動します
5. STA Modeを有効にし、自宅ルーターのSSIDとパスワードを入力します
...
3. ブラウザで`http://10.10.100.254`にアクセスします
4. System Settingsでデフォルトパスワード（admin/admin）でログインします
5. WiFi SettingsでSTA Modeを選択します
6. Scanボタンを押して周辺のAP一覧を読み込みます
...
Communication Settingsで以下のように設定します。
...
Serial Port SettingsでウォールパッドのRS485通信パラメータに合わせて設定します。

...
もう一つは、その資産の上にAppleプラットフォーム統合を一等市民として載せる席はいまだに空いているという点である。AppIntentsとCloudKit、Keychain、BackgroundTasks、NLEmbeddingをコアに抱え込んだフレームワークはまだ見当たらない。モバイルOSの強制終了とバックグラウンド制約、音声インテント、マルチデバイス同期といったモバイル固有の問題は、Appleフレームワークと整合した時に初めてきれいに解決される。Molloが埋める席はその空白である。

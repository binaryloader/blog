...
Apple SiliconでMLXを使ってローカルLLMエージェントフレームワークを自作しながら学習するためのノート集である。
...
学習＋エージェントフレームワーク自作の目的に合わせ、DenseとMoEの2種類を併用する。
...
学習＋エージェントフレームワーク実装の目的において、最もバランスの取れた選択はunsloth UD-MLX-6bitである。
...
エージェントフレームワーク自作のためのステップ別学習ロードマップは以下のとおりである。

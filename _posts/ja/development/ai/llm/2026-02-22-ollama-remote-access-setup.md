...
ただし、Ollamaはデフォルトで`127.0.0.1:11434`でのみリクエストを受け付ける。同じPC上では問題ないが、MacからAPIを呼び出すには外部アクセスを許可する必要がある。普段macOSで開発しているため、Windowsで環境変数を設定してファイアウォールを開けるだけでもかなり苦労した。環境変数を設定したのに動かない場合は、たいていの場合再起動していないのが原因だ。

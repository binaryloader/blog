...
GPUを搭載したWindows PCにOllamaをインストールしてLLM APIサーバーとして活用し、実際の開発やスタディは慣れたmacOS環境で行う構成だ。WindowsはGPUリソースだけを提供し、macOSからAPIを呼び出してLLMを活用する方式で、各デバイスの強みを活かせる。

ただし、Ollamaはデフォルトで`127.0.0.1:11434`でのみリクエストを受け付ける。同じPC上では問題ないが、MacからAPIを呼び出すには外部アクセスを許可する必要がある。

...
Gather Town風モニタリングの構築からこのブログ記事の執筆まで、全プロセスは土曜の夜から約9時間で完了した。サーバーアーキテクチャがすでに堅固だったため、プレゼンテーション層の入れ替えは迅速に進んだ。教訓は、プレゼンテーション層を変更してもデータパイプラインを書き直す必要がないほど、インフラを汎用的に構築すべきだということである。
...
メタ的な話を一つ付け加えると、このブログ記事自体がClaude Codeで書かれた。ダッシュボード開発(2Dオフィスからダッシュボードへの転換)、ボイスメモPoCパイプライン、そしてこの記事の3言語執筆まで、すべてが土曜の夜の約9時間で完了した。もちろん完全に自律的だったわけではない。各フェーズをテストし、フィードバックを与え、方向を定め、エージェントが見逃したものを拾うのは人間の役割だった。ヒューマン・イン・ザ・ループはオプションではなく、アウトプットを使えるものにする核心である。

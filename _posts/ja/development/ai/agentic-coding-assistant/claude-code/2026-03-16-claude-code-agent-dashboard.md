...
メタ的な話を一つ付け加えると、このブログ記事自体がClaude Codeで書かれた。2Dオフィス実験からダッシュボードへの転換、ボイスメモPoCパイプライン、そしてこの記事の3言語執筆まで、すべてが土曜の夜の約9時間で完了した。もちろん完全に自律的だったわけではない。各フェーズをテストし、フィードバックを与え、方向を定め、エージェントが見逃したものを拾うのは人間の役割だった。ヒューマン・イン・ザ・ループはオプションではなく、アウトプットを使えるものにする核心である。

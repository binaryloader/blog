...
最初のアイデアは野心的だった。おそらく野心的すぎた。Gather Town風の2Dバーチャルオフィスを構築し、各エージェントにデスクを与え、ピクセルアートのスプライトで歩き回らせ、性格に基づくアイドル行動を取らせようとした。キム・ソヨン(プロダクトプランナー)はカフェテリアで雑談する社交型、パク・ドヒョン(バックエンド開発者)はデスクでおやつを食べる。COOキャラがCEOのオフィスまで歩いてレポートを届け、エージェントがCOOのデスクに列を作り、完了したエージェントが紙吹雪パーティクルで祝う。

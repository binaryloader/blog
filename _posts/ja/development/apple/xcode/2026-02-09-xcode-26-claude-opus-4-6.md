...
Xcode 26.3ではAgentic Coding AssistantとしてClaude Codeをサポートする。しかしデフォルトで使用されるモデルが最新バージョンではない場合がある。この記事ではXcode 26.3 Release CandidateでClaude Opus 4.6モデルを使用するための設定方法をまとめる。

# 手順

## 1. Claude Codeバイナリのコピー

XcodeのAgentic Coding Assistantは内部的にClaude Codeバイナリを使用する。目的のバージョンのバイナリを以下のパスにコピーする。

```zsh
cp $(which claude) ~/Library/Developer/Xcode/CodingAssistant/Agents/Versions/26.3/claude
```

`claude`バイナリがインストールされていない場合はまずClaude Code CLIをインストールする。
...
モデルをClaude Opus 4.6に変更するために`settings.json`ファイルを作成する。
...
設定が完了したらXcodeを再起動する。Agentic Coding Assistantを使用する際にClaude Opus 4.6モデルが適用される。

### 3.1. ディレクトリ構造

設定完了後のディレクトリ構造は以下の通りだ。

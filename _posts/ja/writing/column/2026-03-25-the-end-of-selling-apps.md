...
前回の記事でバイブコーディングという現象に触れた。コードを直接書かずAIに自然言語で望むものを説明すればソフトウェアが出来上がる方式である。これがわずか数ヶ月の間に爆発的に広がっている。
...
こうしたアプリを運営する企業のビジネスモデルを考えてみよう。ほとんどはフリーミアムモデルだ。基本機能は無料で提供し高度な機能に月額のサブスクリプション料金を取る。あるいは無料ユーザーに広告を表示して広告収入を得る。
...
現在のインターネット経済のかなりの部分は広告で回っている。Google、Meta、数多くの無料アプリやサービスがそうだ。これらのビジネスモデルは本質的に同じだ。ユーザーに無料サービスを提供しそのユーザーの注目を広告主に売る仕組みである。ユーザーのデータを収集して精緻なターゲティング広告を配信する方式だ。これが過去20年間インターネット経済を支配してきたモデルだ。
...
企業の立場からすると、この変化が意味するところは明らかだ。自社の中核となるバーティカルサービスをMCPサーバーとして外に出すことが、新しい流通チャネルを開くことと同じ意味を持つようになるということである。航空券予約、ホテル予約、フードデリバリー、金融取引、医療記録の参照、不動産物件の検索、塾の受講申込といったドメイン特化型の機能をMCPで開いておけば、ユーザーはわざわざ自社アプリを起動しなくても、自分が使っているAIアシスタントの中からそのサービスをそのまま呼び出せるようになる。たとえばユーザーが「今週土曜に釜山行きのKTXを予約して、広安里あたりのホテルも取って」と言えば、コレイルのMCPとホテル予約サービスのMCPが同時に呼び出され、一回の会話の中で二つの取引が完結する。こうしたシナリオはもはや想像の中だけの話ではない。
...
APIエコノミーという概念はすでに数年前から存在していた。Stripe、Twilio、Auth0、SendGrid、Cloudinary、Algoliaがその代表例だ。これらの企業はすでに機能を売るビジネスモデルで成功を収めている。彼らは未来から来たのではなく現在の市場で検証されたモデルだ。

...
ref: til-2026-02-26
excerpt: "Things I learned while studying LangChain's LCEL, prompt templates, message classes, and RunnableGenerator."
...
header:
  ...
  overlay_filter: "0"
  teaser: "/assets/image/thumbnail/teaser/en/til-2026-02-26.png"
categories:
  - Study
  - TIL
tags:
...
Things I learned while studying LangChain's LCEL, prompt templates, message classes, and RunnableGenerator.
...
## 7. UnicodeEncodeError with Korean Input in PyCharm

When running a LangChain chain in PyCharm's console and entering Korean text, you may encounter the following error.

```
UnicodeEncodeError: 'utf-8' codec can't encode character '\udce3' in position 151: surrogates not allowed
```

`\udce3` is the surrogate form of a UTF-8 3-byte sequence (`0xe3`). Due to PyCharm's console encoding settings, Korean input is incorrectly converted to surrogate characters, which then fails UTF-8 encoding when sent to the Ollama API.

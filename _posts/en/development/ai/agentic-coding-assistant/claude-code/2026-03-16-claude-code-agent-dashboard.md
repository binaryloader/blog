...
The Korean names are a deliberate choice. When the COO references "Kim Soyeon" in its orchestration messages, it feels like coordinating with a real team. It also makes the dashboard's sidebar much more readable than a list of IDs like "product-planner" and "server-reviewer-quality".

## 7. Dashboard UI

The dashboard replaced the 2D office with a clean, information-dense layout optimized for monitoring.

### 7.1. Sidebar -- Agent Roster
...
## 8. Real-World Example -- Voice Memo PoC with an Agent Team
...
### 8.1. Phase 1 -- Planning (3 Agents in Parallel)
...
The dashboard immediately shows four cards in Active Tasks -- the COO plus three planners. Kim Soyeon is researching the voice memo market. Lee Junhyuk is designing the technical architecture. Han Yeseul is creating UI wireframes. Each card shows elapsed time ticking up in real time.
...
The planning agents generate design documents in the `docs/` directory. Kim Soyeon's PRD contains a feature priority table and user stories. Han Yeseul's UI design document includes the color palette, wireframes, and interaction flows.
...
### 8.2. Phase 2 -- Development (1 Agent)

The COO reads the planners' outputs and dispatches the web developer. Kang Harin builds the React frontend with Web Speech API for real-time transcription and localStorage for memo persistence. Since this is a browser-native PoC, no backend server is needed.
...
### 8.3. CEO Feedback -- Testing and Iteration
...
The COO receives my feedback, updates its task to "대표님 피드백 반영", and re-dispatches Kang Harin to implement the fixes.
...
### 8.4. Phase 3 -- Review (3 Agents in Parallel)
...
Then something happens. The security auditor (Shin Jaewon) finds two issues -- an XSS vulnerability in the transcript view and unencrypted memo data in localStorage. His card turns red and moves to the Errors column.
...
The COO reads the error, dispatches Kang Harin (the web developer) to fix it. She applies DOMPurify for XSS prevention and encrypts localStorage data with Web Crypto API. Her fix completes and shows in the Completed column.

![Phase 3 -- Security fix applied](/assets/image/post/claude-code-agent-dashboard/07-security-fixed.png)

### 8.5. Phase 4 -- QA & Re-verification (2 Agents in Parallel)

With the security issue resolved, the COO launches QA testing and a security re-audit to verify the fixes. Oh Taeyun runs the full test suite while Shin Jaewon re-audits the patched code.
...
### 8.6. Bug Report -- CEO Finds an Issue
...
The QA engineer confirms: `requestAnimationFrame` stops in inactive tabs and does not restart on return. With the bug confirmed and root cause identified, the COO dispatches Kang Harin to fix it.
...
### 8.7. Phase 5 -- Documentation (1 Agent)

With all verification complete, the COO assigns one final task. Jo Minji (조민지), the technical writer, documents the entire development process.
...
The entire process -- from the dashboard pivot to writing this blog post -- took about 14 hours across a Saturday night and Sunday. The server architecture was already solid, so the presentation layer swap was fast. The lesson: build the infrastructure generically enough that the presentation layer can change without rewriting the data pipeline.
...
This one is subjective, but giving agents Korean names (Kim Soyeon, Park Dohyeon, Shin Jaewon) made the entire experience more engaging. Reading "Shin Jaewon found an XSS vulnerability" is more memorable than "security-auditor found an XSS vulnerability". It turns abstract tool usage into something that feels like team coordination.

The names also help in conversation with Claude Code. When the COO says "dispatching Kim Soyeon for market research", it reads like a natural delegation rather than a function call.

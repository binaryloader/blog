...
ref: til-2026-02-26
excerpt: "LangChain의 LCEL, 프롬프트 템플릿, 메시지 클래스, RunnableGenerator를 학습하면서 배운 것들을 정리한다."
...
header:
  ...
  overlay_filter: "0"
  teaser: "/assets/image/thumbnail/teaser/ko/til-2026-02-26.png"
categories:
  - Study
  - TIL
tags:
...
LangChain의 LCEL, 프롬프트 템플릿, 메시지 클래스, RunnableGenerator를 학습하면서 배운 것들을 정리한다.
...
## 7. PyCharm에서 한국어 입력 시 UnicodeEncodeError

PyCharm 콘솔에서 LangChain 체인을 실행하고 한국어를 입력하면 아래와 같은 에러가 발생할 수 있다.

```
UnicodeEncodeError: 'utf-8' codec can't encode character '\udce3' in position 151: surrogates not allowed
```

`\udce3`는 UTF-8 3바이트 시퀀스(`0xe3`)의 surrogate 형태다. PyCharm 콘솔의 인코딩 설정 문제로 한국어 입력이 surrogate 문자로 잘못 변환되고 Ollama API로 전송될 때 UTF-8 인코딩에 실패하는 것이다.

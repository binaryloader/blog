...
GPU가 장착된 Windows PC에 Ollama를 설치해서 LLM API 서버로 활용하고, 실제 개발과 스터디는 익숙한 macOS 환경에서 하려는 구성이다. Windows는 GPU 자원만 제공하고 macOS에서 API를 호출해 LLM을 활용하는 방식이라 각 기기의 장점을 살릴 수 있다.

...
그럼에도 방향성 자체는 부정하기 어렵다. 이 흐름이 어디로 향하는지에 대해서는 [[Column] 더 이상 개발자 없이는 안 된다는 말은 통하지 않는다](/ko/writing/column/the-end-of-developer-scarcity/)에 더 자세히 썼다. 요약하면 코딩할 수 있는 사람에 대한 수요가 사라지는 게 아니라 "코딩"의 정의가 확장되고 있다. AI 에이전트 팀을 이끄는 것도 여전히 소프트웨어 엔지니어링이다. 다만 우리가 익숙한 모습과 다를 뿐이다.

...
Gather Town 스타일 모니터링 구현부터 이 블로그 포스트 작성까지 전체 과정은 토요일 밤부터 약 9시간 만에 완료됐다. 서버 아키텍처가 이미 탄탄해서 프레젠테이션 레이어 교체는 빠르게 진행됐다. 교훈은 이것이다. 인프라를 충분히 범용적으로 만들어 놓으면 프레젠테이션 레이어를 바꿔도 데이터 파이프라인을 다시 작성할 필요가 없다.
...
한 가지 메타적인 이야기를 덧붙이면 이 블로그 포스트 자체가 Claude Code로 작성되었다. 대시보드 개발(2D 오피스에서 대시보드로 전환), 음성 메모 PoC 파이프라인, 그리고 이 포스트의 3개 언어 작성까지 모든 게 토요일 밤부터 약 9시간에 걸쳐 이루어졌다. 물론 완전 자율은 아니었다. 각 단계를 테스트하고 피드백을 주고 방향을 잡고 에이전트가 놓친 것을 잡는 건 사람의 몫이었다. 사람이 루프 안에 있는 것은 선택이 아니라 산출물을 쓸 수 있게 만드는 핵심이다.

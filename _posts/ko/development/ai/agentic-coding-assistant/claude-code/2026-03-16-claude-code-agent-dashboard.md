...
첫 번째 아이디어는 야심 찼다. 아마 너무 야심 찼다. Gather Town 스타일의 2D 가상 오피스를 만들고 싶었다. 각 에이전트에게 책상을 주고 픽셀 아트 스프라이트로 돌아다니게 하고 성격에 따른 대기 행동을 구현하는 것이었다. 김소연님(기획자)은 카페테리아에서 수다를 떠는 사교적인 캐릭터로 설정했다. 박도현님(백엔드 개발자)은 자리에서 간식을 먹는다. CTO 캐릭터가 CEO 사무실로 걸어가서 보고하고 에이전트들이 CTO 자리 앞에 줄을 서고 작업을 마친 에이전트는 컨페티 파티클로 축하하는 식이었다.
...
| 기획팀 | 개발팀 | 리뷰팀 | QA & 보안 |
|--------|--------|--------|-----------|
| 김소연(제품 기획자) | 강하린님(웹 개발자) | 최유진(웹 아키텍처) | 오태윤님(QA 엔지니어) |
| 이준혁(기술 설계자) | 윤서진(iOS 개발자) | 임수빈(웹 품질) | 신재원(보안 감사관) |
| 한예슬(UI/UX 디자이너) | 박도현님(백엔드 개발자) | 배지훈(iOS 아키텍처) | |
| 조민지님(테크니컬 라이터) | 정우성(인프라 엔지니어) | 송다은(iOS 품질) | |
| | | 황민호(서버 아키텍처) | |
| | | 전지수(서버 품질) | |
| | | 권도윤(인프라 보안) | |
| | | 나영준(인프라 운영) | |

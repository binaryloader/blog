...
이 사이클을 밀어붙이는 데 시간 자체도 만만치 않게 들어갔다. 거의 두 달 가까이 작업이 길어지는 날에는 새벽 늦게까지 사이드 프로젝트와 공부에 붙어 있었다. 본업 일과를 마무리한 늦은 저녁부터 시작해 한국어 토크나이저 빌드 한 번 깨지면 다시 빌드하는 데 몇 시간이 흐르고 임베딩/리랭커 비교 한 라운드 돌리면 또 몇 시간이 사라지는 식이었다. 입지에 대한 불안감으로 시작한 공부였지만 막상 들어가 보니 매일 새벽까지 매달릴 만큼 흥미가 따라왔다. 사이드 프로젝트가 하나의 학습 가속기로 작동한 시간이었다.

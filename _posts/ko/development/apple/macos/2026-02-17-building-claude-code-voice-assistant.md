...
header:
  ...
  overlay_filter: "0"
  teaser: "/assets/image/thumbnail/teaser/ko/building-claude-code-voice-assistant.png"
categories:
  - Development
  - Apple
  - macOS
...
밤에 잠이 안 와서 시작한 데모 프로젝트치고는 ~~생각보다 쓸 만하게 나왔다~~ 생각보다 재밌게 놀았다. "Claude Code에 음성을 입힐 수 있을까?"라는 질문에 대한 답 정도는 됐다.
...
다만 아쉬운 점은 응답 속도다. 음성으로 질문하면 Claude Code가 분석하고 도구를 호출하고 응답을 생성하기까지 시간이 꽤 걸린다. 단순한 질문에도 수 초에서 십수 초를 기다려야 하는데 음성 인터페이스에서 이 대기 시간은 체감이 크다. 개선한다면 이런 방향을 생각해 볼 수 있다. 전체 응답을 기다리지 않고 첫 문장이 완성되는 시점부터 TTS를 시작하는 스트리밍 방식, 간단한 질의에는 Haiku 같은 빠른 모델로 분기하는 방법, 그리고 processing 상태 진입 시점에 오디오 세션을 미리 초기화해서 재생 시작 지연을 없애는 방법 정도가 있다. 지금은 데모 수준이라 여기까지 손대지는 않았지만 스트리밍 TTS만 구현해도 체감이 확 달라질 것 같다.

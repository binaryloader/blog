...
excerpt: "Xcode 26.3에서 Claude Opus 4.6 모델을 에이전틱 코딩 어시스턴트로 설정하는 방법을 정리한다."
...
Xcode 26에서는 에이전틱 코딩 어시스턴트로 Claude Code를 지원한다. 하지만 기본적으로 사용되는 모델이 최신 모델이 아닐 수 있다. 이 글에서는 Xcode 26.3에서 Claude Opus 4.6 모델을 사용하도록 설정하는 방법을 정리한다.

# 정리

## 1. Claude Code 바이너리 복사

Xcode의 에이전틱 코딩 어시스턴트는 내부적으로 Claude Code 바이너리를 사용한다. 원하는 버전의 바이너리를 아래 경로에 복사한다.
...
설정이 완료되면 Xcode를 재시작한다. 에이전틱 코딩 어시스턴트를 사용할 때 Claude Opus 4.6 모델이 적용된다.

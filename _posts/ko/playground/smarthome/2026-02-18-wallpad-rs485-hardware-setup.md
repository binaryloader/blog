...
5. STA Mode를 활성화하고 집 공유기의 SSID와 비밀번호를 입력한다
...
3. 브라우저에서 `http://10.10.100.254`로 접속한다
4. System Settings에서 기본 비밀번호(admin/admin)로 로그인한다
5. WiFi Settings에서 STA Mode를 선택한다
...
Communication Settings에서 다음과 같이 설정한다.
...
Serial Port Settings에서 월패드의 RS485 통신 파라미터에 맞춰 설정한다.

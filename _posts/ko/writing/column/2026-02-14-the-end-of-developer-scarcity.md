...
이 현상에는 이미 이름까지 붙었다. 바로 바이브 코딩이다. OpenAI 공동창업자 Andrej Karpathy가 만든 이 용어는 코드를 직접 작성하는 대신 AI에게 원하는 것을 자연어로 설명하고 생성된 결과를 그대로 받아들이는 방식을 뜻한다. 코드를 읽지 않아도 소프트웨어가 만들어지는 시대가 온 것이다.
